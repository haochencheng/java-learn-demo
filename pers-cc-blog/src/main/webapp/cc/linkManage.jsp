...
<script type="text/javascript">
	...
	function openAddListDialog(){
		...
		url='${pageContext.request.contextPath}/cc/link/save.do';
	}
	...
	function openModifyBlogDialog(){
		...
		$('#fm').form('load',row);
		url='${pageContext.request.contextPath}/cc/link/save.do?id='+row.id;
	}
	...
	function deleteList(){
		...
		$.messager.confirm('系统提示','您确定要删除这<font color="red">'+SelectedRows.length+'</font>条友情链接么!',function(r){
			if(r){
				$.post('${pageContext.request.contextPath}/cc/link/delete.do', {ids:ids}, function(result) {
					if(result.success){
...
<table id="dg" title="博客管理" class="easyui-datagrid" 
  ...
  url="${pageContext.request.contextPath}/cc/link/list.do" fit="true" toolbar="#tb">
	<thead>
		<tr>
